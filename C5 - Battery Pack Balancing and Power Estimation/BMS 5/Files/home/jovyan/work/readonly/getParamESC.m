...
function theParam = getParamESC(paramName,temp,model)
  ...
  % Initialize output
  theParam = 0*temp;
  for k = 1:length(theParam)
    ind = find(model.temps == temp(k)); % see if there is an exact match to
    if ~isempty(ind), % avoid call to (slow) interp1 whenever possible
      if size(theParamData,1) == 1,
        theParam(k) = theParamData(ind);
      else
        theParam(k) = theParamData(ind,:);
      end
    else % if there is not an exact match, we interpolate between parameter
      theParam(k) = interp1(model.temps,theParamData,temp(k),'spline'); % values
